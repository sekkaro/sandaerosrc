...
//		mFundraisingService.saveFundraising(fundraisingDto);
	...
	public String paymentDetail(@PathVariable("no") Long no, @RequestParam(value="pageNum", defaultValue="1") Integer pageNum,
		...
		EventEntity eventId = mPaymentService.getSingleEventEntityById(no);
		List<FundraisingDto> fundraisingDtoList = mFundraisingService.findByEventId(eventId);
		
		// for debugging
		if(!fundraisingDtoList.isEmpty()) {
			System.out.println(fundraisingDtoList.get(0).getMemo());
			System.out.println(fundraisingDtoList.get(0).getDonatorPhone());
		}
